...
logo = Image.open(
                  "data/img/banner_st_pag_red.png"
                 )
st.image(logo,
		   caption="",
		   use_column_width=True) 
...
# Aqui o placehoder vazio finalmente é atualizado com dados do filtered_df 
    ...
    info_sidebar.info("Qtde. Registros:\t\t***{}***".format(data.shape[0])) 
...
# Set Options for page rendering    
...
def get_attributes_data(data):
    ...
    st.markdown(
    "<h6 style='text-align: center; color:#343A40;'> DADOS TRATADOS </h6>", 
    unsafe_allow_html=True)
    
# Análise dados que serão trabalhados nas análises 
            
    if st.sidebar.button('***Clic*** ***Gerar dados*** ℹ️', use_container_width=False) is True:
...
# Atributos dos imóveis e lista para o filtro.
    ...
    # if st.sidebar.checkbox('Mostre o dataset'):
    #    st.write(data.head(5))  
...
# Concatenando o DataFrame criado acima com as análises descritivas 
   ...
   dfm = pd.concat([min_, media, max_, mediana, std],axis=1).reset_index() # Concate pelo eixo das colunas criando novo index
   
   dfm.columns = ['ATRIBUTOS', 'MÍNIMO', 'MÉDIA', 'MÁXIMO', 'MEDIANA', 'DESV.PADRÃO'] 
   # Parâmetros de configuração de colunas quantidade 
...
def get_map(data):
    ...
    mapa.update_layout(mapbox_style='open-street-map') # Estilo do mapa usado
    mapa.update_layout(height=400, width=1000, margin={'r': 0, 'l': 0, 't': 0, 'b': 0}) # Cordenadas do mapa. 
